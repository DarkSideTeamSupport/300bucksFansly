...
async def cmd_settings(message: Message, state: FSMContext) -> None:
	...
	await message.answer(
		...
		reply_markup=account_keyboard(await settings_repo.load()),
		parse_mode="HTML",
	)
...
@router.callback_query(F.data.in_({"menu:account", "menu:migration", "menu:profile", "menu:network"}))
async def cb_account(callback: CallbackQuery, state: FSMContext) -> None:
	await state.clear()
	await edit_menu(
		callback,
		await account_text(),
		account_keyboard(await settings_repo.load()),
	)
