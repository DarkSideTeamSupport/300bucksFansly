...
class TDataConverter:
	...
	async def convert_one(self, session_path: str) -> dict:
		...
		client = TelegramClient(base, api=api)
		try:
	...
	async def convert_many(self, sessions: Optional[List[str]] = None) -> dict:
		...
		for path in paths:
			try:
				results.append(await self.convert_one(path))
